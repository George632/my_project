...
if __name__ == '__main__':
    center = torch.zeros((2, 2, 3), dtype=torch.float32)
    center[:, :, 0] = 1
    center[:, :, 1] = 2
    center[:, :, 2] = 3
    angle = torch.zeros((2, 2), dtype=torch.float32)
    angle[:, 0] = 0
    box_size = torch.zeros((2, 2, 3), dtype=torch.float32)
    box_size[:, :, 0] = 2
    box_size[:, :, 1] = 4
    box_size[:, :, 2] = 6
    center = flip_axis_to_camera_tensor(center)
    corners_3D = get_3d_box_batch_tensor(box_size, angle, center)
    print(corners_3D)