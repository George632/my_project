...
def save_wireframe_model(vertices, edges, wireframe_file):
    r"""
...
class Building3DReconstructionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # ------------------------------- WFTR Ground Truth ------------------------
        wf_edges_points = np.stack((wf_vertices[wf_edges[:, 0]], wf_vertices[wf_edges[:, 1]]), axis=1)
        wf_edges_points = wf_edges_points[
            np.arange(wf_edges_points.shape[0])[:, np.newaxis], np.flip(np.argsort(wf_edges_points[:, :, -1]), axis=1)]
        wf_centers = (wf_edges_points[..., 0, :] + wf_edges_points[..., 1, :]) / 2
        wf_sizes = abs(wf_edges_points[..., 0, :] - wf_edges_points[..., 1, :])
        wf_line_class = (wf_edges_points[:, 0, 0] < wf_edges_points[:, 1, 0]) * 2 + (
                wf_edges_points[:, 0, 1] < wf_edges_points[:, 1, 1])
        wf_line_class[wf_line_class == 2], wf_line_class[wf_line_class == 3] = -1, 2
        wf_line_class[wf_line_class == -1] = 3
        wf_line_class = wf_line_class[:, np.newaxis]
        wf_line_number = wf_edges.shape[0]
        ...
        ret_dict['wf_sizes'] = wf_sizes.astype(np.float32)
        ret_dict['wf_line_class'] = wf_line_class.astype(np.int64)
        ret_dict['wf_line_number'] = wf_line_number
        ret_dict['wf_edges_points'] = wf_edges_points.reshape((-1, 6)).astype(np.float32)
        if self.normalize:
    ...
    def collate_batch(batch):
        ...
        for key, val in input_dict.items():
            try:
                if key in ['wf_vertices', 'wf_edges', 'wf_centers', 'wf_sizes', 'wf_line_class', 'wf_edges_points']:
                    max_len = max([len(v) for v in val])
                    wf = np.ones((len(batch), max_len, val[0].shape[-1]), dtype=val[0].dtype) * np.array(-1e1, dtype=val[0].dtype)
